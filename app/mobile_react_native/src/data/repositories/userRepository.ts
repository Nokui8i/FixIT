...
import { type UserPlatformRole, normalizeStoredUserRole } from "@/shared/domain/userRoles";
...
function toUserProfile(data: unknown, fallbackEmail: string): UserProfileDoc {
  ...
  return {
    ...
    role: normalizeStoredUserRole(d.role),
  };
}
...
export async function updateMyUserProfile(patch: UserProfilePatch): Promise<void> {
  ...
  const db = getFirebaseFirestore();
  const ref = doc(db, "users", user.uid);
  /** Role is server / Console only for staff; clients never persist role via profile edits. */
  const { role: _roleIgnored, ...safePatch } = patch;
