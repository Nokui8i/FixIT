...
import { listenInboxThreads } from "@/data/repositories/messagesRepository";
...
export function MessagesScreen() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      // One-time cleanup of old demo chat artifacts.
      await cleanupMyDemoConversation();
      unsub = await listenInboxThreads((rows) => {
        setThreads(rows);
        setHydrated(true);
      });
    })();
  ...
  const renderRow = ({ item }: { item: ChatThreadRow }) => (
    <Pressable
      ...
      onPress={() =>
        router.push(
          freelancerChatHref(item.proId, {
            proName: item.proName,
            proImageUrl: item.proImageUrl,
          }),
        )
      }
